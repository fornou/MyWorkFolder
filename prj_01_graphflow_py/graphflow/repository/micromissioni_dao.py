...
class MicroMissioniDAO:
    ...
    def get_by_id(self, id_micromissione: int):
        return self.db.query(MicroMissioni).filter(MicroMissioni.ID_Micromissione == id_micromissione).first()

    def get_by_commessa_id(self, id_commessa: int):
        return self.db.query(MicroMissioni).filter(MicroMissioni.ID_Commessa == id_commessa).all()
