...
export async function callAPIwithToken(method, url, body) {
  try {
    ...
    console.log("📡 Fetching:", IP + url, "method:", method);
...
export async function isLogged(callbackOnSuccess) {
  try {
    const response = await callAPIwithToken("GET", "/api/utente/me");
...
export async function logout() {
  try {
    const response = await callAPIwithToken("POST", "/api/auth/logout");
