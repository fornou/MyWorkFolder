...
export async function createCommessa() {
  ...
  try {
    ...
    const response = await callAPIwithToken('POST', '/api/commessa/create',formData);
...
export async function getAllCommesse() {
  ...
  try {
    const response = await callAPIwithToken('GET', '/api/commessa/all');
...
export async function searchCommessa() {
  ...
  try {
    const response = await callAPIwithToken('GET', `/api/commessa/search/${encodeURIComponent(query)}`);
    if (!response.ok) throw new Error("Errore nel recupero dati");
    const commesse = await response.json();
...
export async function getCommessaById() {
  ...
  try {
    const response = await callAPIwithToken('GET', `/api/commessa/${id}`);
    if (!response.ok){ 
    ...
    loadNewData.addEventListener("click", () => {
      window.location.href = `/commessa/${id}/upload-csv`;
    });
