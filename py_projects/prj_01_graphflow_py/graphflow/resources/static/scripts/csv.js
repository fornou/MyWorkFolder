...
export async function setPage() {
  ...
  try {
    const response = await callAPIwithToken("GET", `/api/commessa/${encodeURIComponent(commessaValue)}`);
...
export async function formattaCsv() {
  ...
  try {
    ...
    const response = await callAPIwithToken('POST', `/api/commessa/${encodeURIComponent(commessa)}/${encodeURIComponent(categoria)}/upload`,formData);
